...
reaction_err_t balance_reaction(reaction_t * restrict const react)
{
    ...
    size_t matrix_size;//given the matrix is a nxn matrix, matrix_size shall store the value of n
    ...
    //create matrix
    matrix_size = nvariables > nrows ? nvariables : nrows;
    matrix = malloc(sizeof(ssize_t*)*matrix_size);
    for(size_t i = 0, maxi = matrix_size; i < maxi; i++)
    {
        //create the row
